...
uid_to_human = {}
for line in tf.gfile.GFile('./data/imagenet_synset_to_human_label_map.txt').readlines():
    items = line.strip().split()
    uid_to_human[items[0]] = items[1]


node_id_to_uid = {}
for line in tf.gfile.GFile('./data/imagenet_2012_challenge_label_map_proto.pbtxt').readlines():
...
def create_graph():
    with tf.gfile.GFile('./data/classify_image_graph_def.pb', 'rb') as f:
...
classify_image('./images/test.png')
